namespace Aufgabe2_3_2 {
    function createPartDiv(_part: AlienPart, _index: number): HTMLDivElement {
        ...
        function handleSelection(_e: Event): void {
            ...
            document.cookie = _part.origin + _part; // "part1"
        }
    }
    ...
    function JSONkonvert(_myJSON: string): AllParts {
        let myParts: AllParts = JSON.parse(_myJSON);
        return myParts;
    }
    let myParts: AllParts = JSONkonvert(myJSON);

    showPossibilities(myParts.heads);
